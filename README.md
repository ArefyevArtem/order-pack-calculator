...
→ Response body includes `packs` only: 23×2, 31×7, 53×9429.
...
**5. Tests** — `make test` · `make test-integration` (`-tags=integration`, Docker). **Mocks:** `make mocks` (`.mockery.yaml`).
...
- **One error shape: `{"error":"..."}`** — same JSON for calculator and pack-size errors; successful calculate responses use only `{"packs":[...]}`.
